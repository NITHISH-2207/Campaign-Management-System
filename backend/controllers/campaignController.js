...
joinCampaign: async (req, res) => {
    ...
    getManagerDashboard: async (req, res) => {
        try {
            ...
            // 5. Calculate Period-over-Period Percentage Trends from real MongoDB timestamps
            const now = new Date();
            ...
            // Fetch all posts belonging to manager's campaigns or authored by manager
            const allManagerPosts = await Post.find({
                $or: [
                    { campaignId: { $in: managerCampaignIds } },
                    { authorId: managerId }
                ]
            ...
            const managerPostIds = allManagerPosts.map(p => p._id);

            // A. TOTAL ENGAGEMENT TREND
            const currentPosts = allManagerPosts.filter(p => new Date(p.createdAt) >= currentPeriodStart);
            let currentEngagement = currentPosts.reduce((sum, p) => {
                const eng = p.engagement || {};
                return sum + (eng.likes || 0) + (eng.comments || 0) + (eng.shares || 0) + (eng.views || 0);
            }, 0);

            const previousPosts = allManagerPosts.filter(p => {
                const d = new Date(p.createdAt);
                return d >= previousPeriodStart && d < currentPeriodStart;
            });
            let previousEngagement = previousPosts.reduce((sum, p) => {
                const eng = p.engagement || {};
                return sum + (eng.likes || 0) + (eng.comments || 0) + (eng.shares || 0) + (eng.views || 0);
            }, 0);

            const currentCommentsCount = await Comment.countDocuments({
                postId: { $in: managerPostIds },
                createdAt: { $gte: currentPeriodStart }
            });
            const previousCommentsCount = await Comment.countDocuments({
                postId: { $in: managerPostIds },
                createdAt: { $gte: previousPeriodStart, $lt: currentPeriodStart }
            });

            currentEngagement += currentCommentsCount;
            previousEngagement += previousCommentsCount;

            // B. ACTIVE USERS TREND (unique active users in period)
            const currentSurveyUsers = await SurveyResponse.distinct('user', {
                $or: [{ survey: { $in: managerSurveyIds } }, { campaign: { $in: managerCampaignIds } }],
                completedAt: { $gte: currentPeriodStart }
            });
            const currentModuleUsers = await UserProgress.distinct('userId', {
                moduleId: { $in: managerModuleIds },
                $or: [
                    { completedAt: { $gte: currentPeriodStart } },
                    { lastAccessedAt: { $gte: currentPeriodStart } },
                    { startedAt: { $gte: currentPeriodStart } }
                ]
            });
            const currentCommentUsers = await Comment.distinct('authorId', {
                postId: { $in: managerPostIds },
                createdAt: { $gte: currentPeriodStart }
            });

            const currentActiveUsersSet = new Set([
                ...currentSurveyUsers.map(id => id.toString()),
                ...currentModuleUsers.map(id => id.toString()),
                ...currentCommentUsers.map(id => id.toString())
            ]);

            const previousSurveyUsers = await SurveyResponse.distinct('user', {
                $or: [{ survey: { $in: managerSurveyIds } }, { campaign: { $in: managerCampaignIds } }],
                completedAt: { $gte: previousPeriodStart, $lt: currentPeriodStart }
            });
            const previousModuleUsers = await UserProgress.distinct('userId', {
                moduleId: { $in: managerModuleIds },
                $or: [
                    { completedAt: { $gte: previousPeriodStart, $lt: currentPeriodStart } },
                    { lastAccessedAt: { $gte: previousPeriodStart, $lt: currentPeriodStart } },
                    { startedAt: { $gte: previousPeriodStart, $lt: currentPeriodStart } }
                ]
            });
            const previousCommentUsers = await Comment.distinct('authorId', {
                postId: { $in: managerPostIds },
                createdAt: { $gte: previousPeriodStart, $lt: currentPeriodStart }
            });

            const previousActiveUsersSet = new Set([
                ...previousSurveyUsers.map(id => id.toString()),
                ...previousModuleUsers.map(id => id.toString()),
                ...previousCommentUsers.map(id => id.toString())
            ]);

            const currentActiveUsers = currentActiveUsersSet.size;
            const previousActiveUsers = previousActiveUsersSet.size;

            // C. SURVEY RESPONSES TREND
            const currentSurveyResponses = await SurveyResponse.countDocuments({
                $or: [{ survey: { $in: managerSurveyIds } }, { campaign: { $in: managerCampaignIds } }],
                completedAt: { $gte: currentPeriodStart }
            });
            const previousSurveyResponses = await SurveyResponse.countDocuments({
                $or: [{ survey: { $in: managerSurveyIds } }, { campaign: { $in: managerCampaignIds } }],
                completedAt: { $gte: previousPeriodStart, $lt: currentPeriodStart }
            });

            // D. MODULE COMPLETIONS TREND
            const currentModuleCompletions = await UserProgress.countDocuments({
                moduleId: { $in: managerModuleIds },
                status: 'completed',
                completedAt: { $gte: currentPeriodStart }
            });
            const previousModuleCompletions = await UserProgress.countDocuments({
                moduleId: { $in: managerModuleIds },
                status: 'completed',
                completedAt: { $gte: previousPeriodStart, $lt: currentPeriodStart }
            });

            // Helper to compute percentage change handling edge cases (division by zero)
            ...
            const trends = {
                totalEngagement: calcTrend(currentEngagement, previousEngagement),
                activeUsers: calcTrend(currentActiveUsers, previousActiveUsers),
                surveyResponses: calcTrend(currentSurveyResponses, previousSurveyResponses),
                moduleCompletions: calcTrend(currentModuleCompletions, previousModuleCompletions)
            };
